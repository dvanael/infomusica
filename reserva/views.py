...
from .forms import UsuarioForm, PerfilForm, ProfileEditForm, SolicitacaoForm, StatusForm, PostForm, ChangeGroupForm
from .models import *
...
def send_email_create(instance):
    ...
    for admin in admin_users:
        send_mail(
            subject= 'INFOMUSICA - Nova Solicitação Criada',
            message= f'Uma nova solicitação foi criada por {instance.profile.user.username}.',
            recipient_list= [admin.email],
            from_email= None,
            fail_silently= False
        )
    
def send_email_status(instance):
    if instance.status.name == 'Aprovada':
        status = 'APROVADA'
    elif instance.status.name == 'Rejeitada':
        status = 'REJEITADA'
    else:
        status = 'pognt'

    send_mail(
        subject= f'INFOMUSICA - {status}',
        message= f'SUA SOLICITAÇÂO FOI {status}.',
...
# SAVE FORM
def save_form(request, form, template_name, email):
    data = dict()
...
def solicitacao_create(request):
    ...
    return save_form(request, form, "solicitacao/create.html", email='create')
...
def solicitacao_update(request, pk):
    ...
    return save_form(request, form, "solicitacao/update.html", email=None)
...
def status_update(request, pk):
    ...
    return save_form(request, form, "solicitacao/status-update.html", email='status')
...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid() and profile_form.is_valid():
            ...
            send_mail(
                subject= 'INFOMUSICA - Cadastro Confirmado',
                message= 'Seu cadastro foi confirmado em nosso site! Obrigado!',
...
from .forms import ChangeGroupForm
...
def change_group(request, pk):
    ...
    if request.method == 'POST':
        form = ChangeGroupForm(request.user, request.POST)
        if form.is_valid():
            form.save(user)
            
            return redirect('listar-usuario')
    else:
        form = ChangeGroupForm(request.user)

    context = {
        'form':form,
        'Titulo':'Mudar Permissão de {}'.format(user)
    }
